...
export default function AdminMenuPage() {
  ...
  function openEdit(item: MenuItem) {
    setForm({
      ...
      options: (item.menu_item_options || [])
        ...
        .map((o) => ({ id: o.id, name: o.name, price: String(o.price), is_default: o.is_default })),
    });
    setError("");
    setFormOpen(true);
  }
  ...
  function addOption() {
    setForm((f) => ({
      ...f,
      options: [...f.options, { name: "", price: "", is_default: f.options.length === 0 }],
    }));
  }
  ...
  function removeOption(index: number) {
    setForm((f) => {
      ...
      if (options.length > 0 && !options.some((o) => o.is_default)) {
  ...
  function setDefaultOption(index: number) {
    setForm((f) => ({
      ...f,
      options: f.options.map((o, i) => ({ ...o, is_default: i === index })),
    }));
  }
    ...
    try {
      ...
      if (itemId) {
        ...
        if (cleanedOptions.length > 0) {
          const hasDefault = cleanedOptions.some((o) => o.is_default);
  ...
  async function toggleAvailable(item: MenuItem) {
    const previous = items;
    setItems((its) =>
      its.map((i) => (i.id === item.id ? { ...i, is_available: !i.is_available } : i))
    );
                  ...
                  <span className="text-sm font-semibold">
                    {formatCurrency(
                      item.menu_item_options && item.menu_item_options.length > 0
                        ? (item.menu_item_options.find((o) => o.is_default) || item.menu_item_options[0]).price
                        : item.price
                    )}
                  </span>
                </div>
            ...
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  placeholder={form.options.length > 0 ? "Price (unused — options set the price)" : "Price"}
                  className="w-full rounded-lg border border-outline-variant bg-surface-container-low px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-secondary"
                />
                <select
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-semibold uppercase tracking-wide text-on-surface-variant block">
                    Options {form.options.length > 0 && <span className="text-secondary">(shopper must pick one)</span>}
                  </label>
                  <button
                    type="button"
                    onClick={addOption}
                ...
                {form.options.length === 0 ? (
                  <p className="text-xs text-on-surface-variant">
                    No options yet — the price field above is used as-is (e.g. Small, Large, Half Tray, Full Tray).
                  </p>
                ) : (
                  <div className="space-y-2">
                    {form.options.map((option, i) => (
                      <div key={option.id ?? `new-${i}`} className="flex items-center gap-2">
                        <label className="flex items-center gap-1.5 shrink-0" title="Default option shown on the product card">
                          <input
                            type="radio"
                            name="default-option"
                            checked={option.is_default}
                            onChange={() => setDefaultOption(i)}
                          />
                          <span className="text-[10px] uppercase font-semibold text-on-surface-variant">Default</span>
                        </label>
                        <input
                          value={option.name}
                          onChange={(e) => updateOption(i, { name: e.target.value })}
                          placeholder="Option name (e.g. Large)"
                          className="flex-1 min-w-0 rounded-lg border border-outline-variant bg-surface-container-low px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-secondary"
                        />
                        <input
                          type="number"
                          step="0.01"
                        ...
                        <div className="flex shrink-0">
                          <button
                            ...
                            className="p-1.5 hover:bg-surface-container-low rounded-lg text-on-surface-variant disabled:opacity-30"
                          >
                            ↑
                          </button>
                          <button
