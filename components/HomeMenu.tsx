...
import { buildCartItem, getFromPrice, getSortedOptions } from "@/lib/menuItemOptions";
...
export default function HomeMenu({ categories, items }: { categories: Category[]; items: MenuItem[] }) {
  ...
  function handleAddClick(item: MenuItem) {
    // Items with multiple options need the customer to pick one first, so open the
    // detail modal instead of adding a single fixed price straight to the cart.
    if (getSortedOptions(item).length > 0) {
      setOptionsItem(item);
      return;
    }
    addItem(buildCartItem({ item, selectedOption: null, quantity: 1, image: item.menu_images?.[0]?.url }));
  }
  ...
  return (
    <section id="menu" className="max-w-container-max mx-auto px-4 py-section-gap md:px-gutter">
          ...
          <section key={category.id} aria-labelledby={`home-category-${category.id}`} className="rounded-xl bg-white p-5 shadow-soft">
            ...
            <ul className="divide-y divide-outline-variant/25">
              {categoryItems.slice(0, 3).map((item) => {
                const hasOptions = getSortedOptions(item).length > 0;
                return (
                  <li key={item.id} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 py-3 first:pt-1 last:pb-1">
                    ...
                    <span className="shrink-0 whitespace-nowrap font-semibold text-primary">
                      {hasOptions ? "From " : ""}{formatCurrency(getFromPrice(item))}
                    </span>
                    <button
                      type="button"
      ...
      {optionsItem && (
        <FoodModal item={optionsItem} open={!!optionsItem} onClose={() => setOptionsItem(null)} />
      )}
    </section>
  );
}
