...
export default function HomeMenu({ categories, items }: { categories: Category[]; items: MenuItem[] }) {
  ...
  return (
    <section
      id="menu"
      className={`max-w-container-max mx-auto px-4 md:px-gutter ${hasItems ? "py-section-gap" : "py-12 md:py-16"}`}
    >
      <div className={`flex items-end justify-between gap-4 ${hasItems ? "mb-10" : ""}`}>
        <div>
          <span className="text-sm font-bold tracking-widest uppercase text-secondary">Order with ease</span>
        ...
        <Link
          href="/menu"
          className="shrink-0 rounded-full border border-primary px-4 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary hover:text-on-primary"
        >
          Full menu
        </Link>
      </div>
      ...
      {hasItems && (
        <>
                ...
                <ul className="divide-y divide-outline-variant/25">
                  {categoryItems.slice(0, 3).map((item) => (
                    <li key={item.id} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 py-3 first:pt-1 last:pb-1">
                      ...
                      <span className="shrink-0 whitespace-nowrap font-semibold text-primary">{formatCurrency(getDisplayPrice(item))}</span>
                      <button
                        type="button"
          ...
          {optionItem && (
            <FoodModal item={optionItem} open={!!optionItem} onClose={() => setOptionItem(null)} />
          )}
        </>
      )}
    </section>
